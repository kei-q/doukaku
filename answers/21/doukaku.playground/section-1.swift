...
class Solver {
    typealias InputType = [(String,[Character])]
    typealias OutputType = [Character:[String]]

    func parse(input:String) -> InputType {
        let a = input.componentsSeparatedByString("|")
        return map(a) { s -> (String,[Character]) in
            let c = s.componentsSeparatedByString("_")
            return (c[0], map(c[1]){$0})
        }
    }
    
    func format(result:OutputType) -> String {
        let result_ = map(result) { (k,v) -> String in
            let v_ = sorted(v) { (a, b) in
                a.toInt()! < b.toInt()!
            }
            let v__ = ":".join(v_)
            return "\(k)_\(v__)"
        }
        return "|".join(sorted(result_))
    }
    
    func solve_(input:InputType) -> OutputType {
        var result : OutputType = [:]
        var rest : [(String,[Character])] = []
        var next = input
