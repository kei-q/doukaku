...
func joinMap<C : CollectionType, T>(source: C, transform: (C.Generator.Element) -> [T]) -> [T] {
    return [].join(map(source, transform))
}
...
class Rails {
    ...
    init(deadends : String) {
        input = deadends
        rails = worldmap.filter { (k,_) in !contains(deadends, k) }
    }

    func trace(curr:Character) -> [Character] {
        if let next = rails[curr] {
            return joinMap(next,self.trace)
        } else {
            return [curr]
        }
    }
    ...
    func solve_(start: Character) -> [String] {
        ...
        return filter(goals) { c in contains(deadends,c) }
            .map { c in "\(start)\(c)" }
    }
    
    func solve() -> String {
        let result = joinMap(starts, self.solve_)
