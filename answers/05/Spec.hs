...
main = hspec $ do
    describe "solve" $ do
        forM_ testdata $ \(input, expected) -> do
            it ("check :" ++ input) $ ((==) `on` normalizeTestData) (run input) expected

--normalizeTestData :: String -> String
--normalizeTestData = undefined -- SU.join "," . sort . SU.split ","

-- TODO:
-- 手抜きチェック
normalizeTestData = length
